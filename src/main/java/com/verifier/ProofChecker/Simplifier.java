...
public class Simplifier {
    ...
    public static String simplify(String condition) {
        ...
        condition = normalize(condition);// Normalize the condition (remove unnecessary spaces and brackets)
        condition = simplifyTrivial(condition);// Simplify trivial expressions (true/false removal)
        condition = removeDuplicates(condition);// Remove duplicates in AND/OR clauses
        condition = simplifyNested(condition);// Optimize nested expressions
